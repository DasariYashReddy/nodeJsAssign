...
app.get("/user/tweets/feed/", authenticator, async (request, response) => {
  ...
  const tweetsQuery = ` SELECT username,tweet,date_time as dateTime FROM user as user INNER JOIN (SELECT * FROM tweet WHERE tweet.user_id IN (SELECT following_user_id FROM follower WHERE
...
app.get("/user/following/", authenticator, async (request, response) => {
  const { username } = request;
  const query = `SELECT DISTINCT username as name from follower INNER JOIN user ON follower_user_id = (SELECT user_id FROM user WHERE username='${username}')`;
...
app.get("/user/followers/", authenticator, async (request, response) => {
  const { username } = request;
  const query = `SELECT DISTINCT username as name from follower INNER JOIN user ON following_user_id = (SELECT user_id FROM user WHERE username='${username}')`;
  const result = await db.all(query);
  response.send(result);
});
...
app.get("/tweets/:tweetId/", authenticator, async (request, response) => {
  ...
  const query = `SELECT * from tweet WHERE user_id = (SELECT user_id FROM user WHERE username='${username}')`;
...
app.get("/user/tweets/", authenticator, async (request, response) => {
  const { username } = request;
  const query = `SELECT tweet.tweet_id,tweet,count(distinct like_id) as likes, count(distinct reply_id) as replies, date_time as dateTime FROM tweet INNER JOIN like ON tweet.tweet_id = like.tweet_id
    INNER JOIN reply ON tweet.tweet_id = reply.tweet_id WHERE tweet.user_id = (SELECT user_id FROM user WHERE username = '${username}')`;
